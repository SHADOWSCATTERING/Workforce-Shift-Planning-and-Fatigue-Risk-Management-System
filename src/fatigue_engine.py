...
class FatigueEngine:
    ...
    def __init__(self, conn=None, rules: dict = None):
    ...
    def get_shifts_for_employee(self, employee_id: str, start_date: str = None, end_date: str = None):
        query = "SELECT * FROM shifts WHERE employee_id = ?"
        params = [employee_id]
        if start_date:
    ...
    def detect_overlaps(self, shifts: list) -> list:
        ...
        intervals = [(s, *shift_start_end(s)) for s in shifts]
    ...
    def rest_gaps(self, shifts: list) -> list:
        ...
        if len(shifts) < 2:
            return []
        timed = sorted(
            [(s, *shift_start_end(s)) for s in shifts],
            key=lambda x: x[1],
        )
        gaps = []
    ...
    def consecutive_working_day_streaks(self, shifts: list) -> list:
        ...
        if not shifts:
            return []
        unique_dates = sorted({datetime.strptime(s["shift_date"], "%Y-%m-%d").date() for s in shifts})
        streaks = []
        streak = 1
    ...
    def analyze_employee(self, employee_id: str, start_date: str = None, end_date: str = None) -> dict:
        ...
        for a, b in overlaps:
            violations.append({
                ...
                "severity": "Critical",
                "detail": f"Shift {a['shift_id']} ({a['shift_date']} {a['start_time']}-{a['end_time']}) "
                          f"overlaps with {b['shift_id']} ({b['shift_date']} {b['start_time']}-{b['end_time']}).",
            })
        ...
        for gap in self.rest_gaps(shifts):
            if gap["rest_hours"] < self.rules["min_rest_hours"]:
                ...
                severity = "High"
                detail = (f"Only {gap['rest_hours']}h rest between shift {gap['from_shift']} "
                          f"({gap['from_date']}) and {gap['to_shift']} ({gap['to_date']}); "
        ...
        long_shifts = [s for s in shifts if shift_duration_hours(s) > self.rules["max_single_shift_hours"]]
        for s in long_shifts:
            violations.append({
                "rule_id": "R004", "rule_name": "Maximum Single Shift Length",
                "severity": "Medium",
                "detail": f"Shift {s['shift_id']} on {s['shift_date']} is "
    ...
    def _analyze_shift_list(self, employee: dict, shifts: list) -> dict:
        ...
        for a, b in overlaps:
            violations.append({
                ...
                "detail": f"Shift {a['shift_id']} overlaps with {b['shift_id']} on {a['shift_date']}.",
            })
            score += RISK_WEIGHTS["overlap"]
        ...
        for gap in self.rest_gaps(shifts):
            if gap["rest_hours"] < self.rules["min_rest_hours"]:
                violations.append({
                    ...
                    "detail": f"Only {gap['rest_hours']}h rest between {gap['from_shift']} and {gap['to_shift']}.",
                })
                score += RISK_WEIGHTS["min_rest_hours"]
    ...
    def workforce_risk_summary(self) -> dict:
        employees = self.conn.execute("SELECT employee_id FROM employees").fetchall()
