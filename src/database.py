...
def seed_from_csv():
    ...
    with db_session() as conn:
        conn.executemany(
            """INSERT OR REPLACE INTO employees
               ...
               VALUES (:employee_id, :name, :role, :department, :employment_type,
                       :max_weekly_hours, :contracted_hours, :experience_years, :min_rest_hours_required)""",
            employees,
        )
        conn.executemany(
            """INSERT OR REPLACE INTO shifts
               (shift_id, employee_id, shift_date, shift_type, start_time, end_time, location, department)
               VALUES (:shift_id, :employee_id, :shift_date, :shift_type, :start_time, :end_time, :location, :department)""",
            shifts,
        )
        conn.executemany(
    ...
    print(f"Seeded: {len(employees)} employees, {len(shifts)} shifts, "
          f"{len(availability)} availability rows, {len(fatigue_rules)} fatigue rules.")
