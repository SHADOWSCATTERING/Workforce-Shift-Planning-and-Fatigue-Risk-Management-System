...
- A proper scheduling optimizer (e.g. constraint programming) instead of
  the greedy suggestion engine, to auto-generate full rosters.
- Per-employee/per-jurisdiction configurable rule sets.
- A frontend dashboard (React/Streamlit) consuming this API — the
  `/api/dashboard/risk-summary` and `/api/employees/<id>/fatigue-risk`
  endpoints were designed with this in mind.
...
│
├── tests/
│   └── test_api.py             # 26 unit + integration tests
│
├── requirements.txt
├── .env.example
└── README.md
